...
## 设置时区，同步硬件时钟，设置语言
arch-chroot /mnt bash -c "ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && hwclock --systohc && sed -i 's/#en_US.UTF-8/en_US.UTF-8/' /etc/locale.gen && locale-gen && echo 'LANG=en_US.UTF-8' > /etc/locale.conf && sudo sed -i '/^root/a\wine ALL=(ALL) ALL' /etc/sudoers && exit"
...
## 安装 grub
arch-chroot /mnt bash -c "pacman -Sy && grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id=GRUB && grub-mkconfig -o /boot/grub/grub.cfg && pacman -S networkmanager && exit"
